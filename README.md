...
## Telegram-логи запросов поставщика

После каждого успешного получения и нормализации товаров поставщика скрипт может отправлять короткий отчет в Telegram. Для включения заполните:

```dotenv
TELEGRAM_BOT_TOKEN=123456:telegram-bot-token
TELEGRAM_CHAT_IDS=111111111,222222222
TELEGRAM_NOTIFY_EVERY_SUPPLIER_REQUESTS=1
```

- `TELEGRAM_BOT_TOKEN` — токен бота от BotFather.
- `TELEGRAM_CHAT_IDS` — один или несколько chat ID через запятую.
- `TELEGRAM_NOTIFY_EVERY_SUPPLIER_REQUESTS` — частота отправки. `1` значит отправлять после каждого запроса товаров поставщика, `5` — после каждого пятого.

Счетчик хранится в `data/telegram-state.json`, поэтому частота сохраняется между запусками. Если токен или chat IDs не заполнены, Telegram-уведомления отключены.
